...
# class MLP_conv3(nn.Module):
#     def __init__(
#             self,
#             in_channels,out_channels=None,hidden_channels=None,n_layers=2,non_linearity=F.gelu,dropout=0.0):
#         super().__init__()
#         self.n_layers = n_layers
#         self.in_channels = in_channels
#         self.out_channels = in_channels if out_channels is None else out_channels
...
class MLPLinear_res_old(nn.Module):
    def __init__(self, in_channels, out_channels=None, hidden_channels=None, num_layers=2, non_linearity=F.gelu, dropout=0.0, use_residual=False):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = in_channels if out_channels is None else out_channels
        self.hidden_channels = (
        ...
        self.non_linearity = non_linearity
        self.dropout = (
            nn.ModuleList([nn.Dropout(dropout) for _ in range(self.num_layers+1)])
            if dropout > 0.0
            else None
        )
        self.use_residual = use_residual
...
class FourierFeatureMap(nn.Module):
    def __init__(self,in_feature= 3, num_features=64,device ='cuda', scale=1.0):
        super().__init__()
        self.B = torch.randn(in_feature, int(num_features//2),device=device)*scale
...
class FourierFeatureMapping(nn.Module):
    def __init__(self, input_dim, mapping_size, scale=10.0):
        ...
        self.mapping_size = int(mapping_size//2)
        self.scale = scale
        self.B = torch.randn((input_dim, self.mapping_size)) * scale

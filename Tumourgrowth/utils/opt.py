        ...
        for group in self.param_groups:
            ...
            for param in group['params']:
                ...
                if grad1 is None or grad2 is None:
                    raise ValueError("you have to compute the gradient first and save them to param.grad1 and param.grad2。")
...
# loss_fn2 = nn.MSELoss()
# def lr_scheduler(epoch, max_epochs, grouped_losses):
#     return 1e-3 * (1 - epoch / max_epochs)
#
# def betas_scheduler(epoch, max_epochs, grouped_losses):
#     return (0.9, 0.999)
#
# def group_weights_scheduler(epoch, max_epochs, grouped_losses):
#     total_loss = sum(grouped_losses)
#     if total_loss == 0:
#         return [1.0 / len(grouped_losses)] * len(grouped_losses)
#     return [loss.item() / total_loss for loss in grouped_losses]
# param_scheduler = ParamScheduler(
#     epochs=100,
#     lr_scheduler=lr_scheduler,
#     betas_scheduler=betas_scheduler,
#     group_weights_scheduler=group_weights_scheduler,
#     default_lr=1e-3,
...
# target2 = torch.randn(32, 5)
# for epoch in range(100):
#     def closure(skip_backward=False):
#         optimizer.zero_grad()
#         output = model(input)
#         loss1 = loss_fn1(output, target1)
#         loss2 = loss_fn2(output, target2)
...
#
#     print(f'Epoch {epoch}, lr: {current_lr}, betas: {current_betas}, group_weights: {current_group_weights}')
